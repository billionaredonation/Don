...
export async function saveAdminObject({
  ...
  if (config.category === 'job') {
    ...
    if (selectedType === 'lumber_station' || selectedType === 'mine_station') {
      const ownerId = nextPatch.payload.ownerId || nextPatch.payload.owner_id || null;
      const ownerName = nextPatch.payload.ownerName || nextPatch.payload.owner_name || null;
      const jobBusinessType = selectedType === 'lumber_station' ? 'lumber' : 'mine';
      Object.assign(nextPatch.payload, {
        jobBusiness: true,
        jobBusinessId: String(object.id),
        job_business_id: String(object.id),
        jobBusinessType,
        job_business_type: jobBusinessType,
        legalForm: 'ooo',
        legalFormLabel: 'ООО',
        price: 1_000_000,
        buyable: true,
    ...
    if (selectedType === 'wood_processing_factory') {
      const ownerId = nextPatch.payload.ownerId || nextPatch.payload.owner_id || null;
      const ownerName = nextPatch.payload.ownerName || nextPatch.payload.owner_name || null;
      Object.assign(nextPatch.payload, {
        woodProcessingFactory: true,
        woodProcessingFactoryId: String(object.id),
        wood_processing_factory_id: String(object.id),
        legalForm: 'tov',
        legalFormLabel: 'ТОВ',
        price: 3_500_000,
        buyable: true,
    ...
    if (selectedType === 'tool_assembly_factory') {
      const ownerId = nextPatch.payload.ownerId || nextPatch.payload.owner_id || null;
      const ownerName = nextPatch.payload.ownerName || nextPatch.payload.owner_name || null;
      Object.assign(nextPatch.payload, {
        toolAssemblyFactory: true,
        toolAssemblyFactoryId: String(object.id),
        tool_assembly_factory_id: String(object.id),
        legalForm: 'tov',
        legalFormLabel: 'ТОВ',
        price: 3_500_000,
        buyable: true,
    ...
    if (selectedType === 'coal_power_plant') {
      const ownerId = nextPatch.payload.ownerId || nextPatch.payload.owner_id || null;
      const ownerName = nextPatch.payload.ownerName || nextPatch.payload.owner_name || null;
      Object.assign(nextPatch.payload, {
        coalPowerPlant: true,
        coalPlantId: String(object.id),
        coal_plant_id: String(object.id),
        legalForm: 'tov',
        legalFormLabel: 'ТОВ',
        price: 20_000_000,
        buyable: true,
