export const PLAYER_STAMINA_CONFIG = {
  ...
  drainPerFrame: 0.34,
  // Full recovery takes 3-5 minutes. Hydration selects the point inside
  // that range, so water speeds recovery up without making it instant.
...
export function getStaminaRecoverySeconds(waterValue = 100) {
  const water = clamp(
    Number.isFinite(Number(waterValue)) ? Number(waterValue) : 100,
    0,
    100
  );
  const {
  ...
  if (water >= 60) {
    ...
    return recoverySecondsNormal -
      hydration * (recoverySecondsNormal - recoverySecondsHydrated);
  }

  if (water >= 15) {
    const hydration = (water - 15) / 45;
    return recoverySecondsThirsty -
      hydration * (recoverySecondsThirsty - recoverySecondsNormal);
  }
...
export function getStaminaRecoveryPerFrame(waterValue = 100) {
  return PLAYER_STAMINA_CONFIG.max /
    (getStaminaRecoverySeconds(waterValue) * 60);
}
