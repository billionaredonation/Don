...
export const MAP_OBJECT_TYPES = {
  ...
  metallurgy_factory: {
    ...
    category: MAP_OBJECT_CATEGORIES.JOB,
    label: 'Металлургический завод · предприятие',
    icon: '🔥',
    defaultScale: 1.1,
    defaultRotation: 0,
    defaultAsset: 'job_factory_01',
  ...
  wood_processing_factory: {
    ...
    category: MAP_OBJECT_CATEGORIES.JOB,
    label: 'Деревоперерабатывающий завод · предприятие',
    icon: '🪵',
    defaultScale: 1.1,
    defaultRotation: 0,
    defaultAsset: 'job_factory_01',
  ...
  water_treatment_plant: {
    type: 'water_treatment_plant', category: MAP_OBJECT_CATEGORIES.JOB,
    label: 'Водоочистное сооружение · предприятие', icon: '💧',
    defaultScale: 1.1, defaultRotation: 0, defaultAsset: 'job_factory_01',
...
export function createMapObjectDraft({
  ...
  if (config.type === 'metallurgy_factory') {
  ...
  if (config.type === 'wood_processing_factory') {
  ...
  if (config.type === 'water_treatment_plant') {
    basePayload.waterTreatmentPlant = true;
    basePayload.waterTreatmentPlantId = objectId;
