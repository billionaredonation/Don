...
import { acceptTextileStoreRequest, buyTextileOffer, createTextileStoreRequest, getTextileError, loadTextileExchange, loadTextileRawMarket, publishTextileOffer, sellFarmRawToTextile } from '../textile/textileApi.js';
...
const buyExchangeOffer=(chain,id,businessId)=>chain==='textile'?buyTextileOffer(id,businessId):buyFactoryOffer(id,businessId);
const takeExchangeRequest=(chain,id,factoryId,cityId)=>chain==='textile'?acceptTextileStoreRequest(id,factoryId,cityId):acceptStoreRequest(id,factoryId,cityId);
const publishFactoryOffer=(chain,payload)=>chain==='textile'?publishTextileOffer(payload.factoryId,payload.cityId,payload.productType,payload.quantity,payload.unitPrice):createFactoryOffer(payload);
...
function compatibleStoresForChain(stores,chainId){const chain=productionChain(chainId),expected=chain.storeType==='shop'?'grocery':chain.storeType;return stores.filter(store=>{if(store.chainId&&store.chainId!==chainId)return false;const actual=String(store.businessType||'').trim(),normalized=actual==='shop'?'grocery':actual;return normalized?normalized===expected:store.chainId===chainId;});}
function destinationPickerMarkup(){return `<div class="mn-destination-picker" data-destination-picker hidden><button type="button" class="mn-destination-picker-backdrop" data-destination-picker-cancel aria-label="Закрыть"></button><section role="dialog" aria-modal="true" aria-labelledby="mn-destination-picker-title"><header><span><small>ВАШИ СОВМЕСТИМЫЕ МАГАЗИНЫ</small><h3 id="mn-destination-picker-title">Куда отправить товар</h3></span><button type="button" data-destination-picker-cancel>×</button></header><div class="mn-destination-picker-product" data-destination-picker-product></div><div class="mn-destination-picker-list" data-destination-picker-list></div><footer><button type="button" data-destination-picker-cancel>Отмена</button><button type="button" class="is-primary" data-destination-picker-confirm>Отправить на склад</button></footer></section></div>`;}
function destinationPickerRows(stores,chainId){const chain=productionChain(chainId);return stores.map((store,index)=>`<label class="mn-destination-option"><input type="radio" name="mn-destination-store" value="${esc(store.id||store.businessId)}"${index===0?' checked':''}><i>🏪</i><span><strong>${esc(entityName(store,chain.storeLabel))}</strong><small>${esc(store.cityName||store.cityId||'Город не указан')}${store.publicBusinessId?` · ID ${esc(store.publicBusinessId)}`:''}</small></span><b>Выбрать</b></label>`).join('');}
function dealCard(entry,type,actors){const chainId=entry.chainId||'fruit',isRequest=type==='request',chain=productionChain(chainId),product=productionProduct(chainId,entry.productType),compatibleStores=compatibleStoresForChain(actors.stores,chainId),compatibleFactories=actors.factories.filter(f=>f.chainId===chainId),compatible=isRequest?compatibleFactories:compatibleStores,id=esc(entry.id),actor=entityName(entry,isRequest?chain.storeLabel:chain.factoryLabel);return `<article class="mn-deal-card ${isRequest?'is-request':'is-offer'}" data-chain="${esc(chainId)}">
...
<div data-ex-page="create" hidden><div class="mn-create-grid">${createCard('factory',factories)}${createCard('store',stores)}</div></div>${destinationPickerMarkup()}`;}
