...
function compatibleDestinations(actors,chainId,productType){const allowed=productBuyerTypes(chainId,productType);return [...actors.stores.map(item=>({...item,entityKind:'store'})),...actors.factories.map(item=>({...item,entityKind:'factory'}))].filter(item=>{const type=String(item.entityKind==='factory'?(item.factoryType||item.businessType):(item.businessType==='shop'?'grocery':item.businessType)||'').trim();return allowed.includes(type);});}
