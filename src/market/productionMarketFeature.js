...
const onExternalExchangeOpen=async(event)=>{await open('exchange');const chainId=String(event?.detail?.chainId||'').trim();if(!chainId||modal.hidden)return;const filter=content.querySelector(`[data-chain-filter="${chainId}"]`);if(filter)filter.click();};window.addEventListener('mn:production-exchange-open',onExternalExchangeOpen);
