...
function dealCard(entry,type,actors){const chainId=entry.chainId||'fruit',isRequest=type==='request',chain=productionChain(chainId),product=productionProduct(chainId,entry.productType),compatibleStores=actors.stores.filter(s=>{const type=String(s.businessType||'');return !type||type===chain.storeType||(chain.storeType==='grocery'&&type==='shop');}),compatibleFactories=actors.factories.filter(f=>f.chainId===chainId),compatible=isRequest?compatibleFactories:compatibleStores,id=esc(entry.id),actor=entityName(entry,isRequest?chain.storeLabel:chain.factoryLabel);const destinationOptions=isRequest?'':compatibleStores.map(s=>`<option value="store:${esc(s.id||s.businessId)}" data-city="${esc(s.cityId||'')}" data-public-id="${esc(s.publicBusinessId||'')}">🏪 ${esc(destinationLabel(s,chain.storeLabel))}</option>`).join('');return `<article class="mn-deal-card ${isRequest?'is-request':'is-offer'}" data-chain="${esc(chainId)}">
