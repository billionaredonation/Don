...
function routedChainId(chainId,productType){
  return canonicalProductionChainForProduct(productType,chainId)||String(chainId||'').trim()||'fruit';
}
function productBuyerTypes(chainId,productType){
  const routedChain=routedChainId(chainId,productType),chain=productionChain(routedChain),product=productionProduct(routedChain,productType),fallback=chain.storeType==='shop'?'grocery':chain.storeType;
  return (product?.buyerTypes?.length?product.buyerTypes:[fallback]).filter(Boolean);
}
function compatibleDestinations(actors,chainId,productType){
  const routedChain=routedChainId(chainId,productType);
  const allowed=new Set(productBuyerTypes(routedChain,productType));
  ...
  // Готовая одежда/обувь никогда не должна уходить на заводы.
  // Даже если старый оффер пришёл с неверным industryId/chainId, productType является источником истины.
  if(routedChain==='textile'||String(productType||'').startsWith('textile_')){
    return stores.filter(item=>String(item.businessType||'').trim()==='accessory_store');
  }

  // Готовые инструменты идут только в магазин стройматериалов.
  if(routedChain==='tool_assembly'){
    return stores.filter(item=>String(item.businessType||'').trim()==='construction_store');
  }
...
export function enableProductionMarketFeature({root}){root.insertAdjacentHTML('beforeend',shell());const modal=root.querySelector('[data-production-market]'),content=modal.querySelector('[data-market-content]'),exchangeButton=root.querySelector('[data-exchange-open]');let mode='',busy=false,marketAccess=false,exchangeCache=null,currentExchangeData=null,exchangeStoreScope=null;
const normalizeScopeBusinessType=(value)=>{const type=String(value||'').trim().toLowerCase();return type==='shop'?'grocery':type;};
const offerMatchesStoreScope=(offer)=>{if(!exchangeStoreScope)return true;const chainId=routedChainId(offer?.chainId||'fruit',offer?.productType),allowedChains=Array.isArray(exchangeStoreScope.chainIds)?exchangeStoreScope.chainIds:[];if(allowedChains.length&&!allowedChains.includes(chainId))return false;const targetType=normalizeScopeBusinessType(exchangeStoreScope.businessType);if(!targetType)return true;return productBuyerTypes(chainId,offer?.productType).map(normalizeScopeBusinessType).includes(targetType);};
const applyExchangeScopeFilters=()=>{if(mode!=='exchange'||!currentExchangeData)return;const activeChain=content.querySelector('[data-chain-filter].is-active')?.dataset.chainFilter||'all',allowedChains=Array.isArray(exchangeStoreScope?.chainIds)?exchangeStoreScope.chainIds:[];content.querySelectorAll('[data-chain-filter]').forEach(button=>{const chainId=button.dataset.chainFilter||'all';button.hidden=chainId!=='all'&&allowedChains.length>0&&!allowedChains.includes(chainId);});content.querySelectorAll('.mn-deal-card.is-offer').forEach(card=>{const offerId=card.querySelector('[data-offer-destination-open]')?.dataset.offerDestinationOpen,offer=(currentExchangeData.offers||[]).find(item=>String(item.id)===String(offerId)),chainId=card.dataset.chain||routedChainId(offer?.chainId||'fruit',offer?.productType),chainMatches=activeChain==='all'||chainId===activeChain;card.hidden=!(chainMatches&&offerMatchesStoreScope(offer));});content.querySelectorAll('.mn-deal-card.is-request,.mn-delivery-card').forEach(card=>{card.hidden=activeChain!=='all'&&card.dataset.chain!==activeChain;});};
...
const open=async(next)=>{if(busy)return;if(next==='exchange'&&!marketAccess){try{exchangeCache=await loadUniversalExchange();applyMarketAccess(exchangeCache);}catch(error){console.warn('[market] access retry failed:',error);}if(!marketAccess){toast('Биржа доступна владельцам и управляющим предприятий.','error');return;}}busy=true;mode=next;modal.hidden=false;content.innerHTML='<div class="mn-market-loading">Загружаем предложения…</div>';try{const data=next==='raw'?await loadUniversalRaw():await loadUniversalExchange();if(next==='exchange'){exchangeCache=null;currentExchangeData=data;applyMarketAccess(data);}content.innerHTML=next==='raw'?rawMarkup(data):exchangeMarkup(data);modal.querySelector('[data-market-title]').textContent=next==='raw'?'Продать сырьё':'Биржа готовой продукции';modal.querySelector('[data-market-eyebrow]').textContent=next==='raw'?'КЛАВИША O':'КЛАВИША M';if(next==='exchange')applyExchangeScopeFilters();}catch(e){content.innerHTML=`<div class="mn-market-error">${esc(next==='raw'?getProcurementError(e):getFactoryError(e))}</div>`;}finally{busy=false;}};
const close=()=>{modal.hidden=true;};root.querySelector('[data-raw-market-open]').onclick=()=>{exchangeStoreScope=null;void open('raw');};exchangeButton.onclick=()=>{exchangeStoreScope=null;void open('exchange');};modal.querySelectorAll('[data-market-close]').forEach(b=>b.onclick=close);
const onExternalExchangeOpen=async(event)=>{const detail=event?.detail||{},chainIds=Array.isArray(detail.chainIds)?detail.chainIds.map(value=>String(value||'').trim()).filter(Boolean):[],legacyChain=String(detail.chainId||'').trim();exchangeStoreScope={businessType:normalizeScopeBusinessType(detail.businessType),businessId:String(detail.businessId||'').trim(),cityId:String(detail.cityId||'').trim(),chainIds:chainIds.length?chainIds:(legacyChain?[legacyChain]:[])};await open('exchange');if(modal.hidden)return;const selectedChain=exchangeStoreScope.chainIds.length===1?exchangeStoreScope.chainIds[0]:'all';content.querySelectorAll('[data-chain-filter]').forEach(button=>button.classList.toggle('is-active',button.dataset.chainFilter===selectedChain));content.querySelectorAll('[data-ex-tab]').forEach(button=>button.classList.toggle('is-active',button.dataset.exTab==='offers'));content.querySelectorAll('[data-ex-page]').forEach(page=>page.hidden=page.dataset.exPage!=='offers');applyExchangeScopeFilters();};window.addEventListener('mn:production-exchange-open',onExternalExchangeOpen);
const key=e=>{if(e.key==='Escape'&&!modal.hidden){const picker=content.querySelector('[data-destination-picker]');if(picker&&!picker.hidden)picker.hidden=true;else close();return;}if(e.repeat||/INPUT|TEXTAREA|SELECT/.test(e.target?.tagName||''))return;if(e.code==='KeyO'){e.preventDefault();exchangeStoreScope=null;void open('raw');}if(e.code==='KeyM'&&marketAccess){e.preventDefault();exchangeStoreScope=null;void open('exchange');}};window.addEventListener('keydown',key,true);
