...
function markup() {
  return `<div class="mn-coal-backdrop" data-coal-power-modal hidden><section class="mn-coal-panel">
    ...
    <main>
      ...
      <section data-coal-power-page="warehouse" hidden><h3>Склад и скупка угля</h3><p class="mn-coal-note">Владелец задаёт цену, пополняет бюджет и отдельно запускает или останавливает скупку. Деньги и уголь переводятся одной серверной операцией.</p><div class="mn-coal-warehouse-stats"><article><small>Топливо станции</small><strong data-coal-power-warehouse-stock>0</strong></article><article><small>Бюджет скупки</small><strong data-coal-power-warehouse-cash>0 ₴</strong></article><article><small>Цена за 1 уголь</small><strong data-coal-power-warehouse-price>Не установлена</strong></article></div><div class="mn-coal-offer"><span><strong>⚫ Продать обыкновенный уголь</strong><small data-coal-power-offer-status>УЭС пока не принимает сырьё.</small><b>В вашем инвентаре: <em data-coal-power-player-coal>0</em></b></span><input data-coal-power-sell-quantity type="number" min="1" step="1" value="20"><button type="button" data-coal-power-sell>Продать</button></div><div class="mn-coal-owner-controls" data-coal-power-owner-controls hidden><h3>Управление владельца</h3><div class="mn-coal-procurement-switch"><span><strong data-coal-power-procurement-state>Скупка остановлена</strong><small>Цена и бюджет сохраняются после остановки.</small></span><button type="button" data-coal-power-procurement-toggle>Запустить скупку</button></div><div><label><span>Цена за единицу угля</span><input data-coal-power-buy-price type="number" min="1" step="1" value="1"></label><button type="button" data-coal-power-save-price>Сохранить цену</button></div><div><label><span>Пополнить бюджет скупки</span><input data-coal-power-budget-amount type="number" min="1" step="1" value="100000"></label><button type="button" data-coal-power-add-budget>Пополнить</button></div><div><label><span>Загрузить свой уголь без оплаты</span><input data-coal-power-load-quantity type="number" min="1" step="1" value="10800"></label><button type="button" data-coal-power-load>Загрузить</button></div></div></section>
      ...
      <section data-coal-power-page="contracts" hidden><h3>Контракты с подстанциями</h3><p class="mn-coal-note">Владелец назначает цену кВт·ч и отправляет договор на публичный ID подстанции. После принятия энергия реально списывается из накопителя.</p><div class="mn-coal-contract"><input data-coal-power-target maxlength="80" placeholder="Публичный ID подстанции"><input data-coal-power-price type="number" min="0.01" step="0.01" value="2" placeholder="Цена за кВт·ч"><input data-coal-power-amount type="number" min="20000" max="2000000" value="20000" placeholder="Сумма договора"><button type="button" data-coal-power-contract-create>Предложить договор</button></div><div data-coal-power-contract-list></div></section>
    </main></section></div>`;
}
...
export function enableCoalPowerFeature({ root, cityId } = {}) {
  ...
  function render() {
    ...
    q('[data-coal-power-contract-list]').innerHTML = (state.contracts || []).length ? state.contracts.map((contract) => `<article><span><strong>${esc(contract.targetId)}</strong><small>${contract.status==='active'?'Принят подстанцией':'Ожидает решения'}</small></span><b>${Number(contract.unitPrice || 0).toLocaleString('ru-RU')} ₴/кВт·ч</b><em>${formatBusinessMoney(contract.contractAmount || 0)}</em></article>`).join('') : '<p class="mn-coal-note">Договоров пока нет.</p>';
  }
