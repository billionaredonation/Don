...
function markup() {
  return `<div class="mn-nuclear-backdrop" data-nuclear-modal hidden><section class="mn-nuclear-panel">
    <header><div><small>ГОСУДАРСТВЕННЫЙ ЭНЕРГЕТИЧЕСКИЙ ОБЪЕКТ</small><h2>☢️ Атомная электростанция</h2><p>Реакторный блок → генератор → государственный накопитель → будущие подстанции</p></div><button type="button" data-nuclear-close aria-label="Закрыть">×</button></header>
    ...
    <main>
      ...
      <section data-nuclear-page="contracts" hidden><h3>Государственные договоры поставки</h3><p class="mn-nuclear-note">Договоры подготовлены под будущие подстанции. Администрация назначает стоимость кВт·ч отдельно для каждого получателя.</p><div class="mn-nuclear-contract"><input data-nuclear-target maxlength="80" placeholder="ID подстанции"><input data-nuclear-price type="number" min="0.01" step="0.01" value="2" placeholder="Цена за кВт·ч"><input data-nuclear-amount type="number" min="20000" max="2000000" value="20000" placeholder="Сумма договора"><button type="button" data-nuclear-contract-create>Сохранить договор</button></div><div data-nuclear-contract-list></div></section>
    </main></section></div>`;
}
