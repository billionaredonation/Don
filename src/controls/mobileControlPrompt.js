...
async function requestLandscapeMode() {
  ...
  try {
    ...
    // Fullscreen часто запрещён браузером без пользовательского жеста.
  }

  try {
    await screen.orientation?.lock?.('landscape');
  } catch {
    // На iOS и части Android WebView landscape lock не работает.
  }
}
...
/*
  Нормальная математика для landscape БЕЗ rotate(90deg):
...
export function enableMobileJoystick(
  ...
  function setMovingUi(visible) {
    isMovingNow = visible;

    container.dataset.playerMoving = visible ? 'true' : 'false';

    if (staminaBox) {
      staminaBox.dataset.visible = visible ? 'true' : 'false';
    }
  }

  function updateStaminaUi(visible = isMovingNow) {
    setMovingUi(visible);
