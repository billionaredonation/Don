...
export function getFarmUserErrorMessage(error) {
  ...
  const messages = {
    ...
    PROCUREMENT_BUDGET_NOT_ENOUGH: 'У фермы закончился выделенный бюджет скупа.',
    PROCUREMENT_BUDGET_ABOVE_CASH: 'Нельзя выделить на скуп больше денег, чем есть на счёте фермы.',
